...
router.put('/', AuthMiddleware.withAuth, PermissionsMiddleware.checkFileWithId, (request: Request, response: Response) => {
	...
	UsersMiddlware.getUser(request, (user: IUser, request: Request) => {
			CommentsMiddleware.makeCommentReturnComment(fileId, user._id, lineId, comment, (comment: IComment) => {
				response.status(204).send(), SocketMiddleware.sendCommentUpdate(request, fileId.toString(), comment);
			}, (error: Error) => {
				console.error(error);
...
router.get('/file/:fileId', AuthMiddleware.withAuth, PermissionsMiddleware.checkFileWithId, (request, response) => {
	...
	CommentsMiddleware.getFileComments(fileId, (data: IComment[]) => response.status(200).send(data), (error: Error) => {
		console.error(error);
		response.status(500).send();
	});
});
...
router.delete('/:commentId', AuthMiddleware.withAuth, PermissionsMiddleware.checkComment, (request, response) => {
	...
	CommentsMiddleware.deleteComment(commentId, (comment: IComment) => {
		response.status(200).send(), SocketMiddleware.sendCommentDelete(request, comment.about.toString(), comment);
	}, (error: Error) => {
		console.error(error);
		response.status(500).send();
	});

...
export class CommentDB {
  ...
  /**
	 ...
	 * @return an object, with keys = ids, and values all comments made within that one comment thread.
	 */
  ...
  static async getCommentsByThreadParticipation(userID: string, courseID?: string, onlyReplies = false, params: DBTools = {}) {
    ...
    const userid = UUIDHelper.toUUID(userID); const courseid = UUIDHelper.toUUID(courseID);
    return client.query(`
  ...
  static async getCommentsBySubmissionOwner(submissionOwnerID: string, courseID?: string, onlyReplies = false, params: DBTools = {}) {
    const {client = pool, limit = undefined, offset = undefined, after = undefined, before = undefined} = params;
    const userid = UUIDHelper.toUUID(submissionOwnerID); const courseid = UUIDHelper.toUUID(courseID);
    return client.query(`
  ...
  static async filterComment(comment: Comment & { onlyReplies?: boolean }) {
    ...
    const bodysearch = searchify(body);

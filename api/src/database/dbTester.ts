...
async function DBcommentTest() {
    ...
    const C1 = await promise(C.addComment({
        commentThreadID: uuid0,
        userID: uuid0,
        body: "this is a comment i just made!"
    }), "addComment");
...
async function DBpluginTest() {
    ...
    for (const el of pluginExt.hooks) {
        await promise(P.deleteHook({pluginID: ID, hook: el}), "deletePlugin");
    }
    for (const el of pluginExt.hooks) {
        await promise(P.addHook({pluginID: ID, hook: el}), "addPlugin");
    }

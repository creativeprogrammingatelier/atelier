...
export function FileOverview({match: {params: {submissionId, fileId, tab}}}: FileOverviewProperties) {
    ...
    return (
        <Cached cache={file} wrapper={children => <Frame title="File" sidebar
                                                         search={{submission: submissionId}}>{children}</Frame>}>{
            file =>
                <Frame title={FileNameHelper.fromPath(file.name)} sidebar
                    ...
                    <Jumbotron>
                        ...
                        <Cached cache={submission}>{
                            submission => <p>In project <Link to={submissionPath}>{submission.name}</Link> by <Link
                                to={"/user/" + submission.user.ID}>{submission.user.name}</Link></p>
                        }</Cached>

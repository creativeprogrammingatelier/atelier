...
export function CommentCreator({placeholder = "Write a comment", transparent, large, round, allowRestricted, mentions, sendHandler}: CommentCreatorProperties) {
    ...
    return <Form
        className={"commentCreator" + (large ? " commentCreatorLarge" : "") + (round ? " commentCreatorRound" : "") + (restricted ? " restricted" : "") + (transparent ? " bg-transparent" : "")}>
        <Form.Group>
            <InputGroup className="bg-transparent">
                ...
                />
                <InputGroup.Append>
                    {allowRestricted && <Button onClick={() => setRestricted(!restricted)}>{restricted ?
                        <FiEyeOff size={14} color="#FFFFFF"/> : <FiEye size={14} color="#FFFFFF"/>}</Button>}
                    <Button onClick={handleCommentSubmit} disabled={sending}><FiSend size={14}
                                                                                     color="#FFFFFF"/></Button>
                </InputGroup.Append>
                {mentions &&
                <MentionSuggestions
